    ...
    */
	...
	animateDemo: function() {
		...
		$(".butt").click(function() {
			...
			$("body > style").text("");
			$("#showani").text(_class);
			$(".canvas").children().eq(0).removeClass(_class+' animated').addClass(_class+' animated');
			wait = window.setTimeout(function() {
				$(".canvas").children().eq(0).removeClass(_class+' animated')
			}, 1500);

			//显示代码
			$.getJSON("js/animate.json", function(data) {
				var cssCode = data[0];
				if(!cssCode[animateName]){
					$("#show_ani_code").children(".inner").html(_class);
					return;
		...
		$("#see_demo_css,.see_demo_css").click(function() {
			$(".pop_mask").show();
			$("#show_ani_code").show();
		});
	},
...
Tool.Animation.prototype = {
	...
	showAniCode: function() {
		$("#see_code").on("click", function() {
			...
			txt = txt.replace(/}/g, "<br>}");
			txt = txt.replace(/@-/g, "<br>@-");
			txt = txt.replace(/.canvas/g, "<br>.canvas");
