$(function(){
	...
	var lab={
		...
		getHtml:function(){
			...
			$("#menu").find("a").click(function(){
				...
				$(this).addClass("hover");
				$("#ajaxHtml").load(url+'?2015 #loadHtml',function(response, status, xhr){
					if(status=="error"){
						$("#ajaxHtml").html("页面不存在");
					}
					...
					self.copy();
			        self.openNewWindow();
					window.history.pushState("", "", arr[0]+"?"+url.slice(url.indexOf("/")+1,url.length-5));
				});
	...
	function pageLoad(){
		...
		$("#ajaxHtml").load('animation/'+arr[1]+'.html #loadHtml',function(response, status, xhr){
			if(status=="error"){
				$("#ajaxHtml").html("页面不存在");
			}
			lab.setTextareaCode();
		...
		$(".menu_scroll li a").removeClass('hover').each(function(){
			var url=$(this).attr("href");
			url=url.slice(url.indexOf("/")+1,url.length-5);
			if(url==arr[1]){
				$(this).addClass("hover");
			}
		})
